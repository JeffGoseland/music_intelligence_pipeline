...
| 3. Train only | `python3 scripts/train_emotion_models.py` |
| 4. Predict only | `python3 scripts/run_emotion_predictions.py` |
| 5. Validate only | `python3 scripts/validate_song_features.py` |

---

## Artifacts and paths

- **Features:** `data/processed/song_features.csv` (14 columns).  
...
- **song_features.csv:** 14 required columns, row count, no duplicate `song_id`, numeric dtypes, ≥90% non-NaN tempo and non-unknown key.  
