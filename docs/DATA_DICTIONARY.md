...
| **key_note** | string | Parsed from key | Note letter only: A, B, C, D, E, F, G. `"Unknown"` when key is unknown or unparseable. |
| **key_mode** | string | Parsed from key | `"major"` or `"minor"`. |
| **key_signature** | string | Parsed from key | Accidental: `"sharp"`, `"flat"`, or `"natural"`. |
| **is_major** | bool | Parsed from key | `True` if key_mode is major, else `False`. |
...
3. Merge on song_id (left join); add genre = "unknown"; parse key → key_note, key_mode, key_signature, is_major.
...
**Columns:** All columns from `song_features.csv` (18 columns) plus **arousal** and **valence** from `deam_labels.csv` (20 columns total).
...
**Source:** `scripts/run_emotion_predictions.py` loads `song_features.csv`, runs `models/arousal_xgboost.joblib` and `models/valence_xgboost.joblib` on the 11 feature columns, and writes this file. Train models first with `python3 scripts/train_emotion_models.py`. Used by Phase 3 semantic layer (src/semantic) to tag songs and filter by concept (e.g. High Energy, Calm Focus).
