...
Run from project root: python3 scripts/run_full_pipeline.py [--force] [--fast]
  --force  Run all steps even if a step's checkpoint exists (resume is disabled).
  --fast   Fast training: 5 RandomizedSearchCV iterations per model (for quick iteration).
...
def main() -> int:
    ...
    parser.add_argument(
        "--fast",
        action="store_true",
        help="Fast training: 5 CV iterations per model instead of 48–72.",
    )
    ...
    try:
        ...
        if force or not step_done("train"):
            ...
            all_metrics, run_id, versioned_models_dir = run_step(
                "train",
                lambda: train_all_models(fast=fast_train),
            )
