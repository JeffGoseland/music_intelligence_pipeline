...
def main() -> int:
    ...
    try:
        ...
        if force or not step_done("train"):
            ...
            all_metrics, run_id, versioned_models_dir = run_step("train", train_all_models)
            table = format_all_metrics_table(all_metrics)
            print(table)
            versioned_models_dir.mkdir(parents=True, exist_ok=True)
            (versioned_models_dir / "training_metrics.txt").write_text(table, encoding="utf-8")
            _run_info = {
                "run_id": run_id,
            ...
            print(f"  Models and metrics written to {versioned_models_dir}\n", flush=True)
            mark_done("train")
            ...
            print(f"  Skipping (already done). Using models from {versioned_models_dir}\n", flush=True)
            step("train", "skipped")
        ...
        if force or not step_done("predict"):
            ...
            out = run_step("predict", run_emotion_predictions, models_dir=versioned_models_dir)
