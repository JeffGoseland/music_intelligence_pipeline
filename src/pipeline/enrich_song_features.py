...
Produces data/processed/song_features.csv with columns:
  song_id, 10 DEAM features, tempo_bpm, genre, key,
  key_note, key_mode, key_signature, is_major (parsed from key).
...
from .key_parsing import add_key_derived_columns
...
def run_enrich_pipeline(
    ...
    add_key_derived_columns(merged, key_col="key")

    # Column order: song_id, 10 DEAM, tempo_bpm, genre, key, key_note, key_mode, key_signature, is_major
    key_derived = ["key_note", "key_mode", "key_signature", "is_major"]
    final_cols = (
        ["song_id"] + RICH_DEAM_COLUMNS + ["tempo_bpm", "genre", "key"] + key_derived
    )
